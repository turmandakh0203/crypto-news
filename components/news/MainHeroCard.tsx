...
export default function MainHeroCard({ news }: Props) {
  ...
  return (
    <div ref={ref} className="relative border-b border-[#1c1c1c]">
      ...
      <Link
        href={`/news/${news.slug}`}
        className="group block relative overflow-hidden h-[420px] md:h-[620px]"
      >
        {news.image_url ? (
          <Image
            src={news.image_url}
        ...
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-6 md:px-10 md:pb-10 z-10">
          ...
          <div className="mb-4 reveal-wrap">
            <h1
              className={`reveal-up font-ttNormsPro font-bold text-[36px] md:text-[80px] leading-[1.2] w-full md:w-1/2 text-[#f0ece0] ${v}`}
              style={{
                animationDelay: "0.1s",
                backgroundImage:
