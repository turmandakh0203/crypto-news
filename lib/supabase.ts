...
export async function getHeroNews(): Promise<News | null> {
  const { data, error } = await supabaseServer()
    ...
    .single();
  if (error || !data) {
    console.error("getHeroNews:", error);
    return null;
  }
...
export async function getNewsCategory(
  ...
  if (error) {
    console.error("getNewsCategory:", error);
...
export async function getAllNews(): Promise<News[]> {
  const { data, error } = await supabaseServer()
    ...
    .order("id", { ascending: false });
  if (error) {
    console.error("getAllNews:", error);
    return [];
  }
...
export async function getRelatedNews(
  ...
  const { data, error } = await supabaseServer()
    .from("news")
    .select(
      `id, title, slug, category_id, image_url, lead, tags, published, ${CAT_SELECT}`,
    )
...
export async function getTotalViewCount(): Promise<number> {
  ...
  const { count, error } = await client
    ...
    .select("*", { count: "exact", head: true });
  if (error) {
    console.error("getTotalViewCount:", error);
    return 0;
  }
  return count ?? 0;
}
